...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($contrasena !== $confirmar_contrasena) {
        ...
        $consulta = $conexion->prepare("INSERT INTO usuarios (nombre_usuario, contrasena, email, nombre, apellidos, telefono, tipo_usuario) VALUES (?, ?, ?, ?, ?, ?, ?)");

        if ($consulta->execute([$nombre_usuario, $contrasena_hasheada, $email, $nombre, $apellidos, $telefono, $tipo_usuario])) {
            header("Location: inicio.php");
            exit;
        } else {
            echo "Hubo un problema al registrar el usuario. Por favor, intentlo de nuevo.";
        }
    }
}
?>
...
<body>
<div class="header">
    <h1>Registro de Usuario Restaurante</h1>
</div>
<div class="buttons">
